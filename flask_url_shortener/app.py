...
def index():
    if request.method == "POST":
        ...
        codes = c.execute(
            "SELECT * FROM urls WHERE auto_code=:auto_code OR code=:auto_code",
            {"auto_code": auto_code},
        ).fetchall()
        while len(codes) != 0:
            auto_code = random_str()
        ...
        c.execute(
            "INSERT INTO urls (original_url, auto_code, code, date, timestamp, user_id, click) VALUES (:o_url, :code, :code, :date, :time, :u_id, 0)",
            {
...
def url(code):
    ...
    c.execute(
        "UPDATE urls SET click = :c WHERE url_id=:id",
...
def update():
    if request.method == "POST":
        ...
        codes = c.execute(
            "SELECT * FROM urls WHERE auto_code != :new AND code=:new",
        ...
        c.execute(
            "UPDATE urls SET code=:new WHERE auto_code=:code OR code=:code",
        ...
        url = c.execute(
            "SELECT * FROM urls WHERE url_id=:id", {"id": request.args.get("id")}
        ).fetchall()
        ...
        return render_template(
            "confirm.html", BASE_URL=BASE_URL, code=url[0][3], original_url=url[0][1]
        )
...
def dashboard():
    urls = c.execute(
        "SELECT * FROM urls WHERE user_id=:u_id", {"u_id": session.get("user_id")}
    ).fetchall()
...
def api():
    ...
    if request.args.get("custom") and not request.args.get("url"):
        url = c.execute(
            "SELECT * FROM urls WHERE auto_code = :custom OR code = :custom",
            {"custom": request.args.get("custom")},
        ).fetchall()

        if len(url) == 0:
            return jsonify(
                code=200, description="Your custom code is available as of now"
            )
    ...
    if not request.args.get("custom"):
        ...
        c.execute(
            "INSERT INTO urls (original_url, auto_code, code, click) VALUES (:o_url, :code, :code, 0)",
